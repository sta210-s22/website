...
2.  Let's prepare the data for analysis and modeling.
    -   The variable `Q30` contains the respondent's political party identification.
        Make a new variable that simplifies `Q30` into four categories: "Democrat", "Republican", "Independent", "Other" ("Other" also includes respondents who did not answer the question).
    -   The variable `voter_category` identifies the respondent's past voter behavior.
        Relevel the variable to make rarely/never the baseline level, followed by sporadic, then always.
...
8.  Let's use the model to predict the voting categories.
    ...
    -   How well did the model perform?
        Briefly assess the model performance using 2 - 3 observations from the table and/or visualization to support your response.
