...
6. Are the conditions inference met? List of the conditions, create visualizations and/or summary tables to check the conditions and evaluate whether each condition is met. Include an assessment about each condition and a brief explanation about your conclusion.
...
ew_fit <- multinom_reg() %>%
  set_engine("nnet") %>%
  fit(id ~ lineup + weapon + feature + 
        lineup*weapon + lineup*feature + 
        weapon*feature, data = ew)

ew_fit <- repair_call(ew_fit, data = ew)
```

```{r}
#| echo: false

ew_aug <- augment(ew_fit, new_data = ew) %>%
  mutate(
    correct = if_else(id == "correct", 1, 0),
    foil    = if_else(id == "foil", 1, 0),
    reject  = if_else(id == "reject", 1, 0),
    correct_resid = correct - .pred_correct,
    foil_resid    = foil - .pred_foil,
    reject_resid  = reject - .pred_reject
  )
```
...
par(mfrow = c(2,2))
arm::binnedplot(
  x = ew_aug$.pred_correct,
  y = ew_aug$correct_resid, 
...
arm::binnedplot(
  x = ew_aug$.pred_reject, 
  y = ew_aug$reject_resid, 
...
ew_aug %>%
  group_by(lineup) %>%
  summarise(
    correct_avg = mean(correct_resid),
    foil_avg = mean(foil_resid),
    reject_avg = mean(reject_resid)
  ) %>%
  kable(digits = 10)
