...
library(openintro)   # for the county_2019 dataset
...
## Augmenting the data with the model
...
nc_fits <- map_df(1:10, predict_folds)

p_nc_fits <- ggplot(nc_fits, aes(x = hs_grad, y = .fitted, group = fold)) +
  geom_smooth(method = "lm", se = FALSE, size = 0.3, color = "gray50") +
  scale_x_continuous(labels = label_percent(scale = 1, accuracy = 1)) +
  scale_y_continuous(labels = label_percent(scale = 1, accuracy = 1)) +
  labs(
    x = "High school graduate", y = "Uninsured",
    title = "Predicted uninsurance rate",
    subtitle = "For 10 different testing datasets"
    )
...
-   Fit models to each of the samples
:::
:::
:::
...
set.seed(1234)
county_2019_nc_boots <- map_df(1:10, predict_boots)
...
p_nc_boots <- ggplot(county_2019_nc_boots, aes(x = hs_grad, y = .fitted, group = boot_samp)) +
  ...
  labs(
    x = "High school graduate", y = "Uninsured",
    title = "Predicted uninsurance rate",
    subtitle = "For 10 different bootstrap samples"
    )
