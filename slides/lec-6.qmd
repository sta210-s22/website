...
-   Earlier we computer a CI and conducted a HT via simulation:
...
Once we fir the model, we can use the residuals to estimate the regression standard error (the spread of the distribution of the response, for a given value of the predictor variable):
...
$$
t = \frac{\hat{\beta}_1 - 0}{SE_{\hat{\beta}_1}} = \frac{159.483 - 0}{18.171} = 8.777
$$
...
normTail(L = -8.777, U = 8.777, df = nrow(duke_forest) - 2, xlim = c(-9,9), col = "#D9E3E4")
```
:::
:::
