...
-   HW 5 posted tomorrow, due next Friday
...
-   **Slope:** For each additional square feet, the model predicts the sale price of Duke Forest houses to be higher, on average, by `r dollar(slope)`.

## Sample to population

> For each additional square feet, the model predicts the sale price of Duke Forest houses to be higher, on average, by `r dollar(slope)`.
...
A confidence interval will allow us to make a statement like "*for each additional square feet, the model predicts the sale price of Duke Forest houses to be higher, on average, by `r dollar(slope)`, give or take X dollars.*"
...
## Quantifying the variability of the slope
...
::: question
**Fill in the blank:** For each additional square feet, the model predicts the sale price of Duke Forest houses to be higher, on average, by `r dollar(slope)`, give or take \_\_\_ dollars.
:::
...
df_boot_samples_100_hist <- ggplot(df_boot_samples_100_fit %>% filter(term == "area"), aes(x = estimate)) +
  ...
  labs(x = "Slope", y = "Count",
       title = "Slopes of 1,000 bootstrap samples") +
...
-   We are 95% confident that for each additional square feet, the model predicts the sale price of Duke Forest houses to be higher, on average, by `r dollar(lower)` to `r dollar(upper)`.
...
Alternatively, computer the 95% CI as the point estimate $\pm$ 2 standard deviations of the bootstrap distribution:
...
-   Randomizing to quantify the variability of the slope for the purpose of testing, under the assumption that the null hypothesis is true:
    -   Randomize new samples from the original sample

    -   Fit models to each of the samples and estimate the slope

    -   Use features of the distribution of the randomized slopes to conduct a hypothesis test

## Randomization test {.smaller}

-   For a randomization test, we permute one variable to eliminate any existing relationship between the variables
...
::: columns
::: {.column width="50%"}
-   Each of the observed values for `area` (and for `price`) exist in both the original data plot as well as the permuted `price` plot, but the `area` and `price` are no longer matched for a given house
-   That is, each `price` value is randomly assigned to `area` of a given hourse.
:::
...
duke_forest_rand <- duke_forest %>%
  mutate(
    ...
    price_Randomized = sample(price, size = nrow(duke_forest))
    ) %>%
  select(address, contains("price_"), area)
...
-   Null hypothesis: The true slope is 0.

    Altentative hypothesis:
